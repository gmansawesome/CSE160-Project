...
interface Transport{
   ...
   command socket_t setTestClient(uint8_t srcPort, uint16_t dest, uint8_t destPort, uint16_t transfer);
   command void closeTestClient(uint8_t srcPort, uint16_t dest, uint8_t destPort);
   command void write(uint8_t fd, uint8_t msgLength, uint16_t* msg);
